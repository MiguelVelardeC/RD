    ...
    public class EmailUtilities
    {
        ...
        static public void SendEmail(string to, string from, string subject, string message)
        {
            ...
            try
            {
                ...
                string file = appPath + "Images\\logoMail.png";
            ...
            List<EmailFileParameter> parameters, List<System.Net.Mail.Attachment> theAttachedList)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //string appPath = HttpContext.Current.Request.PhysicalApplicationPath;
                    string appPath = AppDomain.CurrentDomain.BaseDirectory.ToString();
                    string file = appPath + "Images\\logo_2.jpg";
                    LinkedResource logo = new LinkedResource(file);
                    logo.ContentId = "companylogo";
                    int imageHeight = 62;
                    int imageWidth = 72;
        ...
        public static void SendErrorWebServiceUpdate(string message)
        {
            string subject = App.Configuration.Configuration.GetAlianzaErrorMailSubject();
            string body = App.Configuration.Configuration.GetAlianzaErrorMailBody(message);
            string emailTo = App.Configuration.Configuration.GetAlianzaMailTo();
            try
            {
            ...
            {
                log.Error("Error sending email", q);
            }
        }

        public static void SendEmailWebServiceUpdate(DateTime dateTime, string resumen)
        {
            string emailTo = App.Configuration.Configuration.GetAlianzaMailTo();
            SendEmailWebServiceUpdate(emailTo, dateTime, resumen);
        }
        public static void SendEmailWebServiceUpdate(string emailTo, DateTime dateTime, string resumen)
        {
            string subject = App.Configuration.Configuration.GetAlianzaWebServiceUpdateMailSubject();
            string body = App.Configuration.Configuration.GetAlianzaWebServiceUpdateMailBody(dateTime, resumen);
            try
            {
                SendEmail(emailTo, subject, body);
            }
        ...
        /// <summary>
        /// Primero verifica en la base de datos si debe enviar el email, si es el caso entonces
        /// envía el correo.
        /// </summary>
        /// <param name="citaDesgravamenId"></param>
        public static void CheckFirstAndSendEmailLaboCitaConcluidos(int citaDesgravamenId)
        {
            string emailTo = App.Configuration.Configuration.GetNacionalDesgravamenMail();
            
            string body = "";
            CitaDesgravamen objCita = null;
            PropuestoAsegurado objPA = null;
            try
            {
                objCita = CitaDesgravamenBLL.GetCitaDesgravamenById(citaDesgravamenId);
                objPA = PropuestoAseguradoBLL.GetPropuestoAseguradoId(objCita.PropuestoAseguradoId);
                //ConsultaCita objConsulta = ConsultaCitaBLL.GetConsultaCitaById(citaDesgravamenId);
                ProgramacionCita objProgramacion = PropuestoAseguradoBLL.GetProgramacionCita(citaDesgravamenId);
                List<PropuestoAseguradoEstudio> estudios = 
                    CitaDesgravamenBLL.GetListaPropuestoAseguradoEstudioByCitaId(citaDesgravamenId);

                bool examenMedicoOk = false;
                bool laboratorioOk = false;
                if (objCita.NecesitaExamen && objProgramacion.Atendida)
                {
                    body += "La cita " + citaDesgravamenId + " necesitaba examen y ya fue realizado el " + objProgramacion.FechaHoraCitaForDisplay;
                    log.Info("La cita " + citaDesgravamenId + " necesitaba examen y ya fue realizado el " + objProgramacion.FechaHoraCitaForDisplay);
                    examenMedicoOk = true;
                }
                ...
                {
                    laboratorioOk = true;
                    foreach (PropuestoAseguradoEstudio objEstudio in estudios)
                    {
                        int nroArchivos = objEstudio.LaboratorioFiles.Count;
                        if (nroArchivos <= 0)
                        {
                            body += "\n" + "La cita " + citaDesgravamenId + " necesitaba laboratorio de " + objEstudio.NombreProveedor + " y tiene " + nroArchivos + " adjuntos.";
                            log.Info("La cita " + citaDesgravamenId + " necesitaba laboratorio de " + objEstudio.NombreProveedor + " y tiene " + nroArchivos + " adjuntos.");
                            laboratorioOk = false;
                        }
                    }
                }

                if (examenMedicoOk && laboratorioOk) {
                    string subject = "Propuesto Asegurado "+ objPA.NombreCompletoForDisplay + " concluido su proceso de revisión médica";
                    SendEmail(emailTo, subject, body);
                }
            }
            catch (Exception q)
            {
                log.Error("No pudo enviar el correo a Nacional Vida", q);
                SystemMessages.SystemMessages.DisplaySystemErrorMessage("No se pudo enviar el correo a Nacional Vida, avisar que ya se terminó manualmente");
            } 
