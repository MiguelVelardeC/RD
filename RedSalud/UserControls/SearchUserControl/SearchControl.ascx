...
<asp:Panel ID="CSearch" runat="server" DefaultButton="btnSearch">
    ...
    <div class="SearchContainer">
        ...
        <asp:ImageButton ID="btnSavedSearches" runat="server"
            Width="16" Height="16"
            CssClass="SearchButton" 
            AlternateText="<%$ Resources: SearchControl,btnSavedSearches_AlternateText %>" 
            ImageUrl="~/images/neutral/next-item.png" />
            
        <asp:ImageButton ID="btnSearchTools" runat="server"
            Width="16" Height="16"
            CssClass="SearchButton" 
            AlternateText="<%$ Resources: SearchControl,btnSearchTools_AlternateText %>" 
            ImageUrl="~/Images/neutral/configuration02.png" />
            
        <asp:ImageButton ID="btnSearch" runat="server"
            Width="16" Height="16"
            CssClass="SearchButton" 
            AlternateText="<%$ Resources: SearchControl,btnSearch_AlternateText %>" 
            ImageUrl="~/images/neutral/search32.png" 
    ...
    <div class="HelpImage">
        <RedSalud:ContextualHelp ID="chSearch" runat="server"
                Mode="Ajax" 
...
<asp:Panel ID="CSearchErrorPanel" runat="server" style="display:none;">
    <h3>
...
<asp:Panel ID="CSearchHelpPanel" runat="server" style="display:none;">
    <h3>
...
<asp:Panel ID="CSearch_SavedSearches_Panel" runat="server" 
    DefaultButton="btnSavedSearches_Save" style="display:none;">
    ...
    <div class="columnContent">
        ...
        <asp:LinkButton ID="btnSavedSearches_Save" runat="server"
            ...
            EmptyDataText="<%$ Resources:SearchControl,SavedSearchesGrid_Empty %>" ><HeaderStyle CssClass="head" />
            <Columns>
        ...
        <asp:ObjectDataSource ID="savedSearchesDataSource" runat="server" 
            ...
            <SelectParameters>
                ...
                <asp:ControlParameter 
                    ...
                    PropertyName="Value" Type="Int32" /></SelectParameters>
            </asp:ObjectDataSource>
        <div class="clear"></div>
    </div>
</asp:Panel>

<asp:Panel ID="CSearch_SearchTools_Panel" runat="server" style="display: none;"
    class="CSearchSearchToolsPanel">
    <div class="columnHead">
        <asp:Label ID="SearchToolsTitleLabel" runat="server" 
                   Text="<%$ Resources: SearchControl, SearchToolsTitle %>" CssClass="title" />
        <asp:HyperLink ID="SearchToolsCloseLink" runat="server"
            CssClass="commands" 
            NavigateUrl="#"
            Text="<%$ Resources: SearchControl,CloseLink_Text %>">
        </asp:HyperLink></div><div class="columnContent">
        <h3>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#<%=btnSearchTools.ClientID %>").click(function (e) {
            e.preventDefault();

            var savedW = $("#<%=CSearch_SearchTools_Panel.ClientID %>").width();
            var savedH = $("#<%=CSearch_SearchTools_Panel.ClientID %>").height();

            var position = $(this).position();
            var thisHeight = $(this).height();
            $("#<%=CSearch_SearchTools_Panel.ClientID %>").css('top', position.top + thisHeight);
            $("#<%=CSearch_SearchTools_Panel.ClientID %>").css('left', position.left - savedW / 2);
            $("#<%=CSearch_SearchTools_Panel.ClientID %>").show();
        });

        // Handler to close the commands list
        $("#<%=SearchToolsCloseLink.ClientID %>").click(function (e) {
            e.preventDefault();
            $("#<%=CSearch_SearchTools_Panel.ClientID %>").hide();
            return false;
        });
        ...
        $("#<%=CSearch_SearchTools_Panel.ClientID %> .searchTools li").click(function (e) {
