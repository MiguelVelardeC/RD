<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AngularControlPrint.ascx.cs" Inherits="UserControls_AngularControlPrint" %>
<asp:Panel ID="Containter" runat="server">
    ...
    <asp:Panel ID="ReadPanel" runat="server" ng-init="LoadJson()" CssClass="ControlPanel">
        ...
        <ul style="margin-left: 15px;">
            <li ng-repeat="obj in master" style="list-style-position: outside; margin-top: 10px;  text-align: justify;">{{obj.name}}:<br />
                ...
                <div style="width: inherit; border-bottom: 1px solid white; opacity: 0; margin-top: 10px; margin-bottom: 10px;"></div>                                
            </li>
        </ul>
    </asp:Panel>
    <asp:Panel ID="EditPanel" runat="server" ng-init="LoadJson()" Visible="false" CssClass="ControlPanel">
        ...
        <div ng-repeat="obj in master">
                ...
                <div ng-switch-when="group">
                    <div ng-repeat="sub in obj.group" style="margin-left: 20px;">
                        ...
                        <div class="validation">
                            <span id="validation{{replaceSpace(sub.name)}}" class="angularValidation" style="display:none;color: Red;">Este campo no puede exceder los {{getMaxlenght()}} caracteres</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="validation">
                <span id="validation{{replaceSpace(obj.name)}}" class="angularValidation" style="display:none;color: Red;">Este campo no puede exceder los {{getMaxlenght()}} caracteres</span>
            </div>
        </div>
    </asp:Panel>
